...
JPUtil = setmetatable({}, {
    __index = {
        ...
        trim6 = function(s) return s:match '^()%s*$' and '' or s:match '^%s*(.*%S)' end,
        ...
        trimWhitespace = function(str) return str:gsub('%s+', ' ') end,

        -- Return a string with the Q3 colours codes (^0 <-> ^7) stripped out
        StripColours = function(message) return message and string.gsub(message, '%^[0-9]', '') or nil end,
        ...
        deepcopy = function(obj, seen)
            ...
            s[obj] = res
            for k, v in pairs(obj) do res[JPUtil.deepcopy(k, s)] = JPUtil.deepcopy(v, s) end
            return res
        end,
