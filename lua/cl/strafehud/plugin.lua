...
AddListener('JPLUA_EVENT_HUD', function(events)
    ...
    if strafeHUD == 0 then return events end
    ...
    if bestSpeed.lastTime < currTime - 2000 then bestSpeed.speed = 0 end

    if groundSpeed.lastTime < currTime - 2000 then groundSpeed.speed = 0 end
    ...
    if trackJump then
        ...
        if position.z > jumpHigh then jumpHigh = position.z end
