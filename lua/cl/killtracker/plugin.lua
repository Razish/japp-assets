...
local function flrand(lo, hi) return lo + (hi - lo) * math.random() end
...
AddListener('JPLUA_EVENT_UNLOAD', function() if kt.power then SaveDB() end end)

-- track kills/deaths
AddListener('JPLUA_EVENT_PLAYERDEATH', function(victim, methodOfDeath, inflictor)
    if not kt.power or (victim and victim.isBot) or (inflictor and inflictor.isBot) then return end
    ...
    elseif inflictor then
        if inflictor == self then
            ...
            if self.isDueling then
                ...
                Alert(ChatColour.Green .. '+1 duel won against ' .. ChatColour.White .. self.duelPartner.name,
                      ChatColour.Green .. math.floor(kt.db.duelsWon) .. ChatColour.White .. '/' ..
                          math.floor(kt.db.duelsLost))
                -- FIXME: powerduel etc?
            elseif GetGametype() < Gametype.TEAM or victim.team ~= self.team then
                if victim.isBot then
                    ...
                    Alert(ChatColour.Green .. '+1 kill',
                          ChatColour.Green .. math.floor(kt.db.kills) .. ChatColour.White .. '/' ..
                              math.floor(kt.db.deaths))
                end
            end
        elseif victim == self then
            ...
            if self.isDueling then
                ...
                Alert(ChatColour.Red .. '-1 duel loss against ' .. ChatColour.White .. self.duelPartner.name,
                      ChatColour.White .. math.floor(kt.db.duelsWon) .. '/' .. ChatColour.Red ..
                          math.floor(kt.db.duelsLost))
...
local function TogglePower(cmd, args)
    if kt.power then
        ...
        LoadDB()
        kt.power = true
        Alert(ChatColour.Yellow .. 'Killtracker turned ' .. ChatColour.Green .. 'on',
...
AddConsoleCommand('kt_stats', function(cmd, args)
    ...
    if #args > 0 then
        ...
        if kt_spam:GetBoolean() then
            SendServerCommand('say ' .. ChatColour.White .. 'K/D: ' .. ChatColour.Green .. math.floor(kt.db.kills) ..
                                  ChatColour.White .. '/' .. ChatColour.Red .. math.floor(kt.db.deaths) .. ' ' ..
                                  ((kt.db.kills - kt.db.deaths) >= 0 and ChatColour.Green or ChatColour.Red) .. netSign ..
                                  net .. ChatColour.White .. '   W/L: ' .. ChatColour.Cyan .. math.floor(kt.db.duelsWon) ..
                                  ChatColour.White .. '/' .. ChatColour.Yellow .. math.floor(kt.db.duelsLost) .. ' ' ..
                                  ((kt.db.duelsWon - kt.db.duelsLost) >= 0 and ChatColour.Green or ChatColour.Red) ..
                                  duelNetSign .. duelNet)
        else
...
AddConsoleCommand('kt_test', function(cmd, args)
    ...
    local netSign = (net >= 0) and '+' or '-'
    net = math.abs(net)
    Alert(((net >= 0) and ChatColour.Green or ChatColour.White) .. netSign .. string.format('%.6f', net),
    ...
    if kt.power then
        ...
        if alert then
            ...
            elseif alert.time < currTime - sDuration then
                ...
                kt.textbox:Draw(ScreenWidth - (alert.padding * 2.0) - (kt.textbox.width * f[1]), ScreenHeight / 2.0)
            end
        end
        ...
        if showStats then
            ...
            table.insert(lines,
                         ChatColour.Cyan .. ' duel ratio: ' .. ChatColour.White .. string.format('%.3f', duelRatio))
            ...
            for _, line in next, lines do
                ...
                kt.textbox:Draw(0.0, (ScreenHeight / 4.0) + lineOffset)
