...
if gametype >= Gametype.CTF then
    razhud.shaders.redFlag = RegisterShader((gametype == Gametype.CTY) and 'gfx/hud/mpi_rflag_x' or 'gfx/hud/mpi_rflag',
                                            true);
    razhud.shaders.blueFlag = RegisterShader((gametype == Gametype.CTY) and 'gfx/hud/mpi_bflag_ys' or
                                                 'gfx/hud/mpi_bflag', true);
...
AddListener('JPLUA_EVENT_UNLOAD', function() RemapShader('console', 'console', '0') end)
...
local function RenderMeterHorz(x, y, w, h, minValue, maxValue, colour, background, flipped)
    ...
    if minValue == nil then return end

    if minValue > 0 then
        local consume = minValue
        while consume > 0 do
            local percent = (consume / maxValue) * w
            if consume > maxValue then percent = w; end
            local padding = 2
            -- then fill it
            if flipped then x = x + (w - percent) end
            DrawRect(x + padding, y + padding, w - (w - percent) - (padding * 2), h - (padding * 2), colour)
...
AddListener('JPLUA_EVENT_HUD', function(events)
    ...
    if (bit32.band(style, HudEvent.Stats) == HudEvent.Stats) then
        ...
        if self.isAlive == false then goto fail end
        ...
        RenderMeterHorz((ScreenWidth / 2.0) - barWidth * widthRatioCoef, ScreenHeight - (barHeight * 2.0),
                        barWidth * widthRatioCoef, barHeight, hp, 100, hpColour, background, true)
        razhud.text.centered = true
        razhud.text.colour = white
        razhud.text.text = tostring(hp)
        razhud.text:Draw((ScreenWidth / 2.0) - barWidth * widthRatioCoef + (barWidth * widthRatioCoef) / 2.0,
                         ScreenHeight - (barHeight * 2.0))

        -- armor
        RenderMeterHorz((ScreenWidth / 2.0) - barWidth * widthRatioCoef, ScreenHeight - barHeight,
                        barWidth * widthRatioCoef, barHeight, ap, 100, green, background, true)
        razhud.text.text = tostring(ap)
        razhud.text:Draw((ScreenWidth / 2.0) - barWidth * widthRatioCoef + (barWidth * widthRatioCoef) / 2.0,
                         ScreenHeight - barHeight)

        -- force
        RenderMeterHorz(ScreenWidth / 2.0, ScreenHeight - (barHeight * 2.0), barWidth * widthRatioCoef, barHeight, fp,
                        100, lightblue, background, false)
        razhud.text.text = tostring(fp)
        razhud.text:Draw((ScreenWidth / 2.0) + (barWidth / 2.0) * widthRatioCoef, ScreenHeight - (barHeight * 2.0))

        -- ammo / saber
        if wp == Weapon.SABER then
            local styleInfo = saberStyleInfo[saberStyle]
            local padding = 2
            DrawRect(ScreenWidth / 2.0, ScreenHeight - barHeight, barWidth * widthRatioCoef, barHeight, background)
            DrawRect(ScreenWidth / 2.0 + padding, ScreenHeight - barHeight + padding,
                     (barWidth - (padding * 2.0)) * widthRatioCoef, barHeight - (padding * 2.0), styleInfo.colour)
        else
            RenderMeterHorz(ScreenWidth / 2.0, ScreenHeight - barHeight, barWidth * widthRatioCoef, barHeight, ammo,
                            maxAmmo, orange, background, false)
            razhud.text.text = tostring(ammo) .. '/' .. tostring(maxAmmo)
            razhud.text:Draw((ScreenWidth / 2.0) + (barWidth / 2.0) * widthRatioCoef, ScreenHeight - barHeight)
        end
    ...
    if (bit32.band(style, HudEvent.Flags) == HudEvent.Flags) then
        ...
        if self.isAlive == false or gametype < Gametype.CTF then goto fail end
        ...
        if (otherTeamHasFlag) then
            ...
            razhud.text:Draw(leftX + ((iconSize * widthRatioCoef) / 2.0), y - razhud.text.height * 2.0)
        else
    ...
    if (bit32.band(style, HudEvent.Scores) == HudEvent.Scores) then
        ...
        if gametype >= Gametype.TEAM then
            ...
            for _, v in ipairs(list) do
                razhud.text.text = v
                if razhud.text.width > maxlen then maxlen = razhud.text.width end
            end

            for i, v in next, list do
                razhud.text.text = v
                razhud.text.centered = false
                razhud.text:Draw(((ScreenWidth / 2.0) - barWidth * widthRatioCoef) - maxlen,
