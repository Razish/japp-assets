...
local function LoadChatStyles()
    ...
    if chatstyle.styles['current'] == nil or chatstyle.styles[chatstyle.styles['current']] == nil then
        ...
        chatstyle.styles['none'] = {['prefix'] = '', ['suffix'] = ''}
...
local function CreateChatStyle(name, prefix, suffix)
    ...
    chatstyle.styles[name] = {['prefix'] = prefix, ['suffix'] = suffix}
    SaveChatStyles()
end
...
local function RemoveChatStyle(name)
    ...
    if name == 'current' then name = chatstyle.styles[name] end
    if name ~= nil then
...
local function SetChatStyle(name)
    ...
    if name ~= 'current' then chatstyle.styles['current'] = name end
...
AddConsoleCommand('chatstyle_create', function(cmd, args)
    ...
    local prefix = args[2]
    local suffix = ''
    if #args == 3 then suffix = args[3] end
...
AddConsoleCommand('chatstyle_list', function(cmd, args) ListChatStyles(print) end)
