...
function map(func, tbl)
    local newtbl = {}
    for i, v in pairs(tbl) do newtbl[i] = func(v, i, tbl) end
    return newtbl
end
...
function filter(func, tbl)
    local newtbl = {}
    for i, v in pairs(tbl) do if func(v, i, tbl) then newtbl[i] = v end end
    return newtbl
end

-- head( table )
-- e.g: head( {1, 2, 3} ) -> 1
function head(tbl) return tbl[1] end
...
function foldr(func, val, tbl)
    for _, v in pairs(tbl) do val = func(val, v) end
    return val
end

-- reduce( function, table )
-- e.g: reduce( operator.add, {1, 2, 3, 4} ) -> 10
function reduce(func, tbl) return foldr(func, head(tbl), tail(tbl)) end

-- curry( f, g )
-- e.g: printf = curry( io.write, string.format )
--	-> function( ... ) return io.write( string.format( unpack( arg ) ) ) end
function curry(f, g) return function(...) return f(g(table.unpack(arg))) end end
...
function bind1(func, val1) return function(val2) return func(val1, val2) end end
...
is = function(check, expected) return function(...) return check(table.unpack(arg)) == expected end end
...
function expand_args(func) return function(...) return func(arg) end end
