	...
	__index = {
		...
		AngleVectors = function( angles, forward, right, up )
			...
			elseif ( right == true ) then
				return Vector3( (-1.0*sr*sp*cy)+(-1.0*cr*(sy*-1.0)), (-1.0*sr*sp*sy)+(-1.0*cr*cy), -1.0*sr*cp )
			elseif ( up == true ) then
		...
		VectorAngles = function( v )
			...
			if v.y == 0 and v.x == 0 then
				...
				if v.x ~= nil then
					yaw = math.atan2( v.y, v.x ) * 180.0 / math.pi
				elseif v.y > 0.0 then
					yaw = 90.0
				else
					yaw = 270.0
				...
				pitch = math.atan2( v.z, forward ) * 180.0 / math.pi
				if pitch < 0 then
					pitch = pitch + 360.0
				end
			end
