...
-- constructor
function Channel.new( name, password )
...
function Channel:__tostring()
	return self.name .. '(' .. #self.clients .. ' clients)'
end
...
function Channel:IsMember( ply )
	return self.clients[ply.id] == true
end
...
function Channel:Message( ply, msg )
	...
	for clientNum,_ in next,self.clients do
		local prefix = (ply == nil) and
			('<' .. ChatColour.Yellow .. 'system~' .. ChatColour.White .. tabEC .. JPUtil.StripColours(self.name) .. tabEC .. '>') or
			('<' .. ply.name .. ChatColour.White .. '#' .. tabEC .. JPUtil.StripColours(self.name) .. tabEC .. '>')
...
function Channel:Leave( ply )
	...
	if self:IsMember( ply ) == false then
		return false
	end
