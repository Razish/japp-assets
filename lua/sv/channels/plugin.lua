...
local function IsLegacyClient( ply )
	return bit32.band( ply.supportFlags, CSF.CHAT_FILTERS ) ~= CSF.CHAT_FILTERS
end

-- lazy constructor
local function GetChannel( name, password )
	for channelName,channel in next,chans.channels do
...
local function GetJoinedChannels( ply )
	local result = {}
	local joinedChans = filter(
		function( channel ) return channel:IsMember( ply ) end,
		chans.channels
	)
	for channelName,channel in next,joinedChans do
...
local function ListJoinedChannels( ply )
	...
	local channels = table.concat(
		map(
			function( channel ) return (channel.name==selectedChan) and (ChatColour.Green .. channel.name .. ChatColour.White) or channel.name end,
			GetJoinedChannels( ply )
		),
		', '
	)
...
local function SelectNextChannel( ply )
	...
	for channelName,channel in next,joinedChans do
		numJoinedChannels = numJoinedChannels + 1
	end
...
AddClientCommand( '.joinchan', function( ply, args )
	...
	elseif channel.password and channel.password ~= args[2] then
		ply:ConsolePrint( ChatColours.Red .. 'invalid password!\n' )
		return
	end
...
AddClientCommand( '.whoischan', function( ply, args )
	...
	local members = table.concat(
		map(
			function( id ) return GetPlayer( id ).name end,
			channel:GetMembers()
		),
		', '
	)
...
AddClientCommand( '.nextchan', function( ply, args )
	SelectNextChannel( ply )
end )

AddClientCommand( '.prevchan', function( ply, args )
	SelectPrevChannel( ply )
end )
...
-- list channels
AddClientCommand( '.chanlist', function( ply, args )
	ListJoinedChannels( ply )
end )
